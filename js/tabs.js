document.addEventListener('DOMContentLoaded', function () {
    // Support legacy, dl-, de- and dc- prefixed tabs
    const tabs = document.querySelectorAll('.tab, .dl-tab, .de-tab, .dc-tab');
    const tabContents = document.querySelectorAll('.tab-content, .dl-tab-content, .de-tab-content, .dc-tab-content');
    // Prefer the container that exists for the current page
    const tabsContainer = document.querySelector('.tabs, .dl-tabs, .de-tabs, .dc-tabs');
    ...
    function activateTab(targetTab) {
        // Remove all known active-class variants from all tabs
        tabs.forEach(tab => {
        ...
        document.querySelectorAll('.note-hover-tooltip.active, .dl-note-hover-tooltip.dl-active, .de-note-hover-tooltip.de-active, .dc-note-hover-tooltip.dc-active').forEach(tooltip => {
        ...
        if (targetTab.classList.contains('dl-tab')) {
        ...
        // Try content selectors for legacy and prefixed variants
        ...
        let activeTab = document.querySelector('.tab.active') || document.querySelector('.dl-tab.dl-active') || document.querySelector('.de-tab.de-active') || document.querySelector('.dc-tab.dc-active') || tabs[0];
        if (activeTab) {
    ...
    window.addEventListener('resize', () => {
        const currentActiveTab = document.querySelector('.tab.active, .dl-tab.dl-active, .de-tab.de-active, .dc-tab.dc-active');
        if (currentActiveTab) {
